...
## Files
- `docker-compose.yml` – Stack definition. Prometheus config is embedded via a Compose `config` (no external file dependency). cAdvisor stays internal (no host port) since Prometheus scrapes it over the network. Prometheus and Grafana are routed via Traefik (no direct host ports).
